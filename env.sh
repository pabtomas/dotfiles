...
ENTRYPOINT_COMPONENT_ID="$(component_id "${ENTRYPOINT_ID}")"
...
ENTRYPOINT_SERVICE="${ENTRYPOINT_ID}"
GIT_SERVICE="${GIT_ID}"
...
ENTRYPOINT_COMPONENT_TAG='latest'
...
ENTRYPOINT_COMPONENT_IMG="$(image "${OWNER_ID}" "${ENTRYPOINT_COMPONENT_ID}" "${ENTRYPOINT_COMPONENT_TAG}")"
...
ENTRYPOINTD_PATH="$(dirname "${ENTRYPOINT_PATH}")/$(basename -s '.sh' "${ENTRYPOINT_PATH}").d"
