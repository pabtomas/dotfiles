...
let s:tick_time = 100
let s:nb_ticks = 100
let s:elapsed_time = s:nb_ticks * s:tick_time

let s:size_buffers = 1

function! s:UpdateBuffersList(timer_id)
  if s:elapsed_time < s:nb_ticks * s:tick_time
    let s:elapsed_time = s:elapsed_time + s:tick_time
    redraw
    set cmdheight=1
...
let s:timer =
  \ timer_start(s:tick_time, function('s:UpdateBuffersList'), {'repeat': -1})
...
" buffers navigation
nnoremap <silent> <Tab> :call StartTimer() <bar>
  \ :call BuffersListNavigation(1)<CR>
nnoremap <silent> <S-Tab> :call StartTimer() <bar>
  \ :call BuffersListNavigation(-1)<CR>
...
"     Buffers Autocommand Groups ----------------------------------------{{{
  ...
  autocmd BufAdd * :let tmp =
    \ len(filter(range(1, bufnr('$')), 'buflisted(v:val)')) | echoe "tmp = " . tmp . ", size_buffers = " .s:size_buffers |
    \ if s:size_buffers != tmp | let s:size_buffers = tmp |
    \ call StartTimer() | unlet tmp | endif
  autocmd BufDelete * :let tmp =
    \ len(filter(range(1, bufnr('$')), 'buflisted(v:val)')) - 1 | echoe "tmp = " . tmp . ", size_buffers = " .s:size_buffers |
    \ if s:size_buffers != tmp | let s:size_buffers = tmp |
    \ call StartTimer() | unlet tmp | endif
