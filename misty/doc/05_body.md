...
- **required**: false
- **default**: `[]`
- **description**: Misty uses as many process as possible and runs a task as soon as possible. So this is here that you can schedule the Misty execution. You can let this list empty but that means that you do not mind that the matching request runs first. This attribute takes a list of ID. Misty will run the request after the tasks listed here will end their execution.
...
- **required**: false
- **default**: `true`
- **description**: By default, if the request failed, Misty stops its execution in failure. If you want to change this behavior for a request, set this attribute to `false`.
...
- **required**: false
- **default**: false
- **description**: A virtual Body will not result as a request execution by Misty. It is particularly useful when you want to reuse common attributes between Bodies.
